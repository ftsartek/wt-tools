...
from src.wt_tools import replays_db
from src.wt_tools.replays_db.replays_db import SectionTransformer
...
@pytest.fixture()
def units_json():
    return {"unit0": {"rank": 0, "tier": 1}, "unit1": {"rank": 7, "tier": 2}}
...
def replays_json():
    return {
        "0": {
            ...
            "start_time": 0,
            "info": {
                "0": {"team": 1, "rank": 0, "units": ["unit0"]},
                "1": {"team": 2, "rank": 7, "units": ["unit1"]},
            },
        }
    }
...
def section_tables():
    return {
        "info": {0: {"team": 0, "rank": 0, "units": ["unit0"]}},
        "users": {0: "root"},
        "units": {"unit0": {"rank": 0, "tier": 1}},
    }
