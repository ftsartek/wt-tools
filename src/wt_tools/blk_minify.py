...
def minify(blk_data: AnyStr, minify_options: Dict[AnyStr, bool]) -> AnyStr:
    ...
    blk_parser = Lark(
        open(os.path.join(get_tool_path(), grammar_path)).read(),
        parser="lalr",
        transformer=blk_transformer(minify_options),
...
def main():
    ...
    parser.add_argument(
        "-O", dest="out_filename", default=False, nargs="?", help="output file"
    )
    # removes not all empty objects, really
    parser.add_argument(
        "--strip_empty_objects",
        dest="strip_empty_objects",
        action="store_true",
        default=False,
        help="remove empty objects",
    )
    # only comment objects, inlined one, like `comments:t=""` not removed
    parser.add_argument(
        "--strip_comment_objects",
        dest="strip_comment_objects",
        action="store_true",
        default=False,
        help="remove comment objects",
    )
    parser.add_argument(
        "--strip_disabled_objects",
        dest="strip_disabled_objects",
        action="store_true",
        default=False,
        help="remove disabled objects",
    )
    parser.add_argument(
        "--strip_all",
        dest="strip_all",
    ...
    with open(out_filename, "w", encoding="utf8") as f:
        f.write(parsed_data)
    print(
        "minified {} from {} to {}, with rate {:.2}".format(
            filename,
            parsed_file_size,
            len(parsed_data),
